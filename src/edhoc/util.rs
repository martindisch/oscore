...
mod tests {
    ...
    const SUITE: isize = 0;
    const X_U: [u8; 32] = [
        0xB1, 0xA3, 0xE8, 0x94, 0x60, 0xE8, 0x8D, 0x3A, 0x8D, 0x54, 0x21,
        0x1D, 0xC9, 0x5F, 0x0B, 0x90, 0x3F, 0xF2, 0x05, 0xEB, 0x71, 0x91,
        0x2D, 0x6D, 0xB8, 0xF4, 0xAF, 0x98, 0x0D, 0x2D, 0xB8, 0x3A,
    ];
    const C_U: [u8; 1] = [0xC3];
    const MESSAGE_1: [u8; 38] = [
        ...
        0x2D, 0xB8, 0x3A, 0x41, 0xC3,
    ];

    const X_V: [u8; 32] = [
        0x8D, 0xB5, 0x77, 0xF9, 0xB9, 0xC2, 0x74, 0x47, 0x98, 0x98, 0x7D,
        0xB5, 0x57, 0xBF, 0x31, 0xCA, 0x48, 0xAC, 0xD2, 0x05, 0xA9, 0xDB,
        0x8C, 0x32, 0x0E, 0x5D, 0x49, 0xF3, 0x02, 0xA9, 0x64, 0x74,
    ];
    const C_V: [u8; 1] = [0xC4];
    const TH_2_INPUT: [u8; 74] = [
        0x01, 0x00, 0x58, 0x20, 0xB1, 0xA3, 0xE8, 0x94, 0x60, 0xE8, 0x8D,
        0x3A, 0x8D, 0x54, 0x21, 0x1D, 0xC9, 0x5F, 0x0B, 0x90, 0x3F, 0xF2,
        0x05, 0xEB, 0x71, 0x91, 0x2D, 0x6D, 0xB8, 0xF4, 0xAF, 0x98, 0x0D,
        0x2D, 0xB8, 0x3A, 0x41, 0xC3, 0x58, 0x20, 0x8D, 0xB5, 0x77, 0xF9,
        0xB9, 0xC2, 0x74, 0x47, 0x98, 0x98, 0x7D, 0xB5, 0x57, 0xBF, 0x31,
        0xCA, 0x48, 0xAC, 0xD2, 0x05, 0xA9, 0xDB, 0x8C, 0x32, 0x0E, 0x5D,
        0x49, 0xF3, 0x02, 0xA9, 0x64, 0x74, 0x41, 0xC4,
    ];
    const TH_2: [u8; 34] = [
        0x58, 0x20, 0x55, 0x50, 0xB3, 0xDC, 0x59, 0x84, 0xB0, 0x20, 0x9A,
        0xE7, 0x4E, 0xA2, 0x6A, 0x18, 0x91, 0x89, 0x57, 0x50, 0x8E, 0x30,
        0x33, 0x2B, 0x11, 0xDA, 0x68, 0x1D, 0xC2, 0xAF, 0xDD, 0x87, 0x03,
        0x55,
    ];
    ...
    const AEAD_NONCE: [u8; CCM_NONCE_LEN] = [
    ...
    const AEAD_AD: [u8; 8] = [0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07];
