...
fn main() {
    ...
    let msg1_sender =
        Msg1Sender::new(u_c_u, u_priv, &u_auth_priv, &u_auth_pub, u_kid);
    ...
    let msg1_receiver =
        Msg1Receiver::new(v_c_v, v_priv, &v_auth_priv, &v_auth_pub, v_kid);
    ...
    assert_eq!(
        u_master_secret,
        [
            0x59, 0x98, 0x02, 0x98, 0x3A, 0x9D, 0x94, 0xA2, 0x73, 0xA1, 0x26,
            0x67, 0x65, 0xD9, 0xEA, 0xD5,
        ]
    );
    assert_eq!(
        u_master_salt,
        [0x1B, 0xE4, 0x2D, 0x91, 0x0D, 0x33, 0x21, 0x2D]
    );
